<?php
	...
	if(isset($_POST['semester_id']) && isset($_POST['class_semester_id'])){
		...
		global $wpdb;
		$dbstudentIDs = $wpdb->get_results( "SELECT student_id FROM syn1_syn_enrollment WHERE class_semester_id = $classSemesterID ");
		
		foreach($dbstudentIDs as $dbstudentID){
			if($dbstudentID->student_id != $studentID){
